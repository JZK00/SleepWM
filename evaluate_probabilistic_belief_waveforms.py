...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate SleepWM probabilistic structural waveform forecasts."
    )
...
def plot_representative(
    ...
    title="SleepWM probabilistic waveform structure under a 120-s all-sensor interruption",
    model_label="SleepWM",
):
    target = torch.cat(records["target"]).float()
    base = torch.cat(records["base"]).float()
    ...
    for values, color, style in (
        ...
        centered = values[selected, eeg_index] - values[selected, eeg_index].mean()
        power = torch.fft.rfft(centered).abs().square().clamp_min(1e-8)
        axes[0, 1].plot(frequency, 10.0 * torch.log10(power), color=color, linestyle=style, linewidth=0.9)
    ...
    axes[0, 0].set_title("EEG probabilistic structural forecast")
    axes[0, 1].set_title(f"EEG spectrum | forecast error {eeg_candidate[selected]:.3f} vs deterministic {eeg_base[selected]:.3f}")
    axes[1, 0].set_title("EMG probabilistic structural forecast")
    axes[1, 1].set_title(f"EMG envelope | forecast error {emg_candidate[selected]:.3f} vs deterministic {emg_base[selected]:.3f}")
...
def main() -> int:
    ...
    model_label = str(protocol.get("display_label", "SleepWM"))
